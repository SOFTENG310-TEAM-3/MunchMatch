...
describe('ExampleButtons Component', () => {
  ...
  it('calls onButtonClick with correct type for each button', () => {
    ...
    expect(onButtonClickMock).toHaveBeenCalledWith('cafe');

    const burgerButton = getByAltText('burger');
    fireEvent.click(burgerButton);
    expect(onButtonClickMock).toHaveBeenCalledWith('burger');

    const sushiButton = getByAltText('sushi');
    fireEvent.click(sushiButton);
    expect(onButtonClickMock).toHaveBeenCalledWith('sushi');

    const fruitButton = getByAltText('fruit');
    fireEvent.click(fruitButton);
    expect(onButtonClickMock).toHaveBeenCalledWith('fruit');
  ...
  it('calls onButtonClick with a random type for "Surprise Me!"', () => {
    ...
    const validTypes = ['cafe', 'burger', 'sushi', 'fruit', 'dessert', 'pizza', 'chicken', 'bakery'];
    expect(validTypes).toContain(calledType);
  });
